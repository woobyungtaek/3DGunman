...
namespace ASEParser
{
	...
	struct COneTVertex
	{
		COneTVertex() : m_u(0.0f), m_v(0.0f), m_w(0.0f) {};
	...
	struct ShapeLine
	{
		...
		// 초기화를 위한 생성자
		ShapeLine() : m_line_number(0), m_shape_closed(false), m_shape_vertexcount(0) {};
	};
	...
	public:
		Scenedata() :
			m_firstframe(0), m_lastframe(0),
			m_framespeed(0), m_ticksperframe(0),
			m_meshframestep(0), m_keyframestep(0) {};
	...
	public:
		...
		int					m_numsubmtls;
		ASEMaterial*		m_submaterial;	

		MaterialMap* m_map_temp;	
		MaterialMap* m_map_diffuse;		
	...
	struct Vertex
	{
		Vertex() :
			...
			m_bw1(0.0f), m_bw2(0.0f), m_bw3(0.0f),
			m_indices(0),
			m_indicesPointer(nullptr)
		{};
		...
		// weight 벡터
		vector<Weight*>	m_bone_blending_weight;
	};
	...
	public:
		Mesh() :
			m_type(0), m_isAnimated(false),
			m_Animation(nullptr),

			m_isparentexist(false),

			m_tm_rotangle(0.0f), m_tm_scaleaxisang(0.0f),
			m_parentMesh(nullptr),

			m_timevalue(0.0f), m_numbone(0), m_numskinweight(0),
			m_mesh_numvertex(0), m_mesh_numfaces(0),
			m_mesh_numtvertex(0),
			m_mesh_tvfaces(0), m_mesh_numcvertex(0),
			m_mesh_facenormal(nullptr), m_mesh_vertexnormal(nullptr),
			m_opt_index(nullptr),

			m_shape_linecount(0),

			m_prop_motionblur(0), m_prop_castshadow(0),
			m_prop_recvshadow(0), m_material_ref(0),

			m_camera_near(0.0f), m_camera_far(0.0f),
			m_camera_fov(0.0f), m_camera_tdist(0.0f),
			m_camera_isloadTarget(false),

			m_light_uselight(false), m_light_useglobal(false),
			m_light_absmapbias(false), m_light_overshoot(false),

			m_lightanimation(nullptr),
			m_light_isloadTarget(false), m_light_isAnimate(false),

			m_is_skinningobject(false),
			m_bone(nullptr),
			m_wvertex(nullptr)
		{
	...
	public:
		...
		Matrix m_AniMatrix;			// 로컬을 대체할 애니매트릭스
		Animation* m_Animation;      // 애니메이션
		...
		///----------------------------------
		Mesh* m_parentMesh;
		vector<Mesh*> m_childMesh;
		Matrix m_WorldTM;	// 현재의 월드 매트릭스
